...
export default class BaseController extends Reporter(EventEmitter) {
    ...
    async _notifyStarted(descriptor) {
        this._observersFor(descriptor).forEach(observer => this._notify('install', () => observer.started(descriptor)));
    }

    async _notifyStopped(descriptor) {
        this._observersFor(descriptor).forEach(observer => this._notify('install', () => observer.stopped(descriptor)));
    }

    async _notifyUninstall(descriptor) {
        this._observersFor(descriptor).forEach(observer => this._notify('install', () => observer.uninstalled(descriptor)));
    }
